...
void CGameRules::m_pEffectShedule()
{
    m_pEffectList.erase(
        std::remove_if( m_pEffectList.begin(), m_pEffectList.end(),
			[]( CBaseEntity* entity ){ return entity == nullptr || !entity->IsAlive(); } ), m_pEffectList.end() );
		...
		for( size_t e = 0; e < entity->effects.size(); ++e )
		{
			...
			while( it != entity->effects.end() )
			{
				...
				if( effect->should_update )
				{
					if( auto player = ToBasePlayer( entity ); player != nullptr )
					{
						MESSAGE_BEGIN( MSG_BROADCAST, gmsgEffect, nullptr, player );
							WRITE_STRING( std::string( it->first ).c_str() );
							WRITE_STRING( std::string( effect->name ).c_str() );
							WRITE_BYTE( effect->end );
						MESSAGE_END();
					}
				...
				*/
				...
				if( effect->time > 0 )
				{
					if( gpGlobals->time > effect->last_time )
					{
						DoEffect = true;
						effect->last_time = gpGlobals->time + effect->time;
					}
				}
				else
				{
